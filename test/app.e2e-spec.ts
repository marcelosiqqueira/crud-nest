...
describe('AppController (e2e)', () => {
  ...
  it('Registrar um novo usuário administrador', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/register')
      .send({...authRegisterDTO, role: Role.Admin, email: 'marceloteste@gmail.com'})
  ...
  it('Validar se a Role do novo usuário ainda é User', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
      .set('Authorization', `bearer ${acessToken}`)
      .send();
